...
echo "<div class='form'>";
echo "FRIEND FEED:<br/><br/>";
$q = mysql_query("SELECT * FROM `FriendList` where `uid`='$userid' and `status`='1'") or die(mysql_error());

while ($row = mysql_fetch_assoc($q)){
$fid = $row['uid_friend'];


$r = mysql_query("SELECT * FROM `Media` m natural join `userList` u where uid='$fid' order by UploadTime Desc Limit 5") or die(mysql_error());
